...
export function LoginForm({ onSuccess, onForgotPassword, onRegister }: LoginFormProps) {
  ...
  const { regionInfo, loading: regionLoading } = useRegion() // Get detected IP from region context
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Get client IP from region context (already detected by RegionProvider)
      // This is more accurate than server-side detection because it can detect VPN/real IP
      const clientIP = regionInfo?.ip && regionInfo.ip !== 'unknown' ? regionInfo.ip : null

      console.log('[LOGIN FORM] Sending login request for:', email, 'with clientIP:', clientIP, 'regionInfo:', regionInfo)
      const response = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({ 
          email, 
          password,
          clientIP: clientIP, // Send detected IP to backend
        }),
      })
  ...
  return (
    <Card className="w-full max-w-md">
      ...
      <CardContent className="space-y-4">
        {/* OAuth Login Buttons - Show based on region */}
        {/* China region: Show WeChat only */}
        {/* Global region: Show Google only */}
        {/* Only show buttons after region detection is complete (not loading and IP is detected) */}
        {!regionLoading && regionInfo && regionInfo.ip !== 'unknown' ? (
          <div className="grid grid-cols-1 gap-3">
            {regionInfo.isChina ? (
              // China region: Show WeChat login
              <Button
                type="button"
                variant="outline"
              ...
              // Global region: Show Google login
              <Button
                type="button"
                variant="outline"
          ...
          <div className="flex items-center justify-center py-2">
            ...
            <span className="ml-2 text-sm text-muted-foreground">Detecting region...</span>
          </div>
        ) : null}

        {/* Divider - Only show if OAuth buttons are shown and region is detected */}
        {!regionLoading && regionInfo && regionInfo.ip !== 'unknown' && (
